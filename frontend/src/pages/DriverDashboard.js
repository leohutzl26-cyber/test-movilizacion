    ...
    <div className="max-w-4xl mx-auto animate-slide-up">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">Control de incidentes y combustible</p>
        </div>
      </div>
            ...
            <form onSubmit={handleIncidentSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={incidentForm.vehicle_id} onValueChange={v => setIncidentForm({...incidentForm, vehicle_id: v})}>
                    <SelectTrigger className="h-12"><SelectValue placeholder="Seleccione patente..." /></SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={incidentForm.incident_type} onValueChange={v => setIncidentForm({...incidentForm, incident_type: v})}>
                    <SelectTrigger className="h-12"><SelectValue placeholder="Seleccione tipo..." /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={incidentForm.severity} onValueChange={v => setIncidentForm({...incidentForm, severity: v})}>
                    <SelectTrigger className="h-12"><SelectValue placeholder="Seleccione gravedad..." /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="baja" className="text-emerald-600">Baja (Solo informar)</SelectItem>
                      <SelectItem value="media" className="text-amber-600">Media (Requiere revisión)</SelectItem>
                      <SelectItem value="alta" className="text-red-600">Alta (Habilita fuera de servicio)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <div className="space-y-2">
                <Label className="font-bold text-slate-700">Descripción de la Novedad</Label>
                <textarea className="w-full min-h-[120px] p-4 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-amber-500 outline-none transition-all shadow-sm bg-slate-50/50" value={incidentForm.description} onChange={e => setIncidentForm({...incidentForm, description: e.target.value})} placeholder="Detalle lo ocurrido o el estado detectado..." />
              </div>
              <Button type="submit" disabled={submitting} className="w-full bg-amber-600 hover:bg-amber-700 text-white h-14 text-lg font-black shadow-lg rounded-xl">
                {submitting ? "Enviando..." : "Registrar en Bitácora"}
              </Button>
            </form>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <form onSubmit={handleFuelSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="font-bold text-slate-700">Kilometraje Actual</Label>
                  <Input type="number" className="h-12 font-bold" value={fuelForm.mileage} onChange={e => setFuelForm({...fuelForm, mileage: e.target.value})} placeholder="Ej: 125400" />
                </div>
                <div className="space-y-2">
                  <Label className="font-bold text-slate-700">Litros</Label>
                  <Input type="number" step="0.01" className="h-12 font-bold" value={fuelForm.liters} onChange={e => setFuelForm({...fuelForm, liters: e.target.value})} placeholder="Ej: 45.5" />
                </div>
                <div className="space-y-2">
                  <Label className="font-bold text-slate-700">Monto ($)</Label>
                  <Input type="number" className="h-12 font-bold" value={fuelForm.amount} onChange={e => setFuelForm({...fuelForm, amount: e.target.value})} placeholder="Ej: 55000" />
                </div>
                <div className="space-y-2">
                  <Label className="font-bold text-slate-700">N° de Boleta / Factura</Label>
                  <Input className="h-12 font-bold" value={fuelForm.receipt_number} onChange={e => setFuelForm({...fuelForm, receipt_number: e.target.value})} placeholder="Ej: 001234" />
                </div>
              </div>
              <Button type="submit" disabled={submitting} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white h-14 text-lg font-black shadow-lg rounded-xl mt-4">
